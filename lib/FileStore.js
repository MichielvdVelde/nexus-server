...
class FileStore {
   ...
   */
  createReadStream (resource, mode = FileStore.MODE_READ, options = { start: 0 }) {
    debug(`createReadStream ('${resource}', '${mode}')`)
    return new Promise((resolve, reject) => {
      if (this._options.maxDepth > 0 && (resource.split('/').length - 1) > this._options.maxDepth) {
        return reject(new Error(`maximum resource depth (${this._options.maxDepth}) exceeded`))
      }
      ...
      fs.access(resourcePath, fs.constants.R_OK, err => {
        ...
        resolve(fs.createReadStream(resourcePath, { flags: mode, start: options.start || 0 }))
      })
    })
  }
   ...
   */
  createWriteStream (resource, mode = FileStore.MODE_WRITE, options = { start: 0 }) {
